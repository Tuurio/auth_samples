...
function settings() {
  ...
  if (issuer.protocol !== 'https:' && !(issuer.protocol === 'http:' && loopback)) {
    throw new Error('TUURIO_ISSUER must use HTTPS outside an explicit loopback host.');
  }
  return {
    issuer,
    clientId: required('TUURIO_CLIENT_ID'),
    clientSecret: env.TUURIO_CLIENT_SECRET?.trim() || undefined,
    redirectUri: redirectUri('TUURIO_REDIRECT_URI'),
    postLogoutRedirectUri: redirectUri('TUURIO_POST_LOGOUT_REDIRECT_URI'),
    scope: env.TUURIO_SCOPE?.trim() || 'openid profile email'
  };
}
...
export async function beginLogin() {
  ...
  transactions.set(transactionId, { verifier, state, expiresAt: Date.now() + 10 * 60_000 });
...
export async function completeLogin(currentUrl: URL, transactionId: string | undefined) {
  ...
  const profile = await oidc.fetchUserInfo(await config(), tokens.access_token, claims.sub);
  ...
  sessions.set(sessionId, {
    ...
    expiresAt: Date.now() + Math.min(tokens.expiresIn() ?? 3600, 3600) * 1000
  });
