...
npx manage-tuurio-id@latest init --framework react --project-dir . --base-url http://localhost:5173 --auth browser --yes --output json --campaign lovable_starter --no-open
...
```bash
npx manage-tuurio-id@latest init \
  --framework react \
  --project-dir . \
